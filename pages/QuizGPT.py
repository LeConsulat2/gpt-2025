...
with st.sidebar:
    ...
    if not st.session_state.openai_api_key:
        ...
        if st.session_state.openai_api_key:
            st.success("API Key saved. You can navigate to other sections now.")
...
if not st.session_state.openai_api_key:
    ...
    llm = OpenAI(api_key=st.session_state.openai_api_key, temperature=0.5)

    if navigation == "App":
        st.title("Welcome to the Main App")
        st.success("API Key authenticated! You can switch to QuizGPT from the sidebar.")

    if navigation == "QuizGPT":
        st.title("QuizGPT")
